export default async function handler(req, res) {
    ...
    if (!DEEPSEEK_API_KEY || !SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
        return res.status(500).json({ action: 'answer', message: '❌ Configuration serveur manquante.' });
    }

    const MAX_RECURSION = 2;

    // Helper pour appeler Supabase (REST)
    ...
    try {
        // Récupérer les données nécessaires (comptes, catégories, 3 dernières transactions, historique 5 messages)
        const [accounts, categories, recentTransactions, chatHistoryRaw] = await Promise.all([
            ...
            supabaseGet('transactions', `user_id=eq.${userId}&select=*,categories(name,icon),accounts(name)&order=created_at.desc&limit=3`),
            supabaseGet('chat_history', `user_id=eq.${userId}&select=role,content&order=created_at.desc&limit=5`)
        ]);
...
- Réponds UNIQUEMENT en JSON, sans texte avant/après.`;
        ...
        // Gestion fetch_transactions / fetch_categories avec rappel
        if (instruction.action === 'fetch_transactions' && recursionDepth < MAX_RECURSION) {
            ...
            const transactions = await supabaseGet('transactions', params);
            ...
            instruction = await callDeepSeek(systemPrompt + '\n\n' + newContext, newHistory, "Maintenant, utilise ces données pour exécuter les actions demandées par l'utilisateur.");
        }
        ...
        // Sauvegarde historique (optionnelle, ignore erreurs)
        await supabasePost('chat_history', { user_id: userId, role: 'user', content: message });
        await supabasePost('chat_history', { user_id: userId, role: 'assistant', content: instruction.message || JSON.stringify(instruction), action: instruction.action });
